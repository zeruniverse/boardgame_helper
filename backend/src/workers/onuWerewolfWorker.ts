...
class OnuWerewolfWorker extends BaseGameWorker {
      ...
      switch (actionType) {
        ...
        case 'restart_game':
          if (playerId !== this.room.hostId) {
            throw new Error('只有房主可以重新开始游戏');
          }
          if (this.gameState.status !== OnuWerewolfGameStatus.COMPLETED) {
            throw new Error('只有游戏结束后才能重新开始');
          }
          this.resetGame();
          break;
        case 'change_config':
  ...
  /** 生成终局完整牌面。终局复盘必须向所有玩家展示全部角色与中心牌。 */
  private getFinalVision(_viewerId?: string): OnuWerewolfVision {
  ...
  private processNextSkill(): void {
    ...
    // 每次推进夜序时先把所有客户端的旧操作权限统一清空，再只向当前玩家
    // 私发 ready 事件。这样即使客户端在重连或上一技能多步交互中保留了旧状态，
    // 也不会出现两个角色同时显示“等待你的行动”。事件不携带当前玩家身份，
    // 因而不会泄露夜间角色顺序。
    Object.values(this.gameState.players).forEach(gamePlayer => {
      gamePlayer.skillReady = false;
    });
    this.sendToRoom('onu_skill_locked', {});
  ...
  private getVisibleGameResult(_viewerId?: string): any {
