...
export class BOTCWorker extends BaseGameWorker {
  ...
  private validateNightActionSubmission(player: GamePlayer, action: NightAction): string | null {
    ...
    switch (roleId) {
      ...
      case 'philosopher': {
        if (
          hasUsedOncePerGameAbility ||
          player.reminders.includes('Philosopher used') ||
          player.reminders.includes('Is the Philosopher')
        ) return null;
  ...
  private async handleDeathAbilityAction(playerId: string, data: any): Promise<void> {
    ...
    if (abilityRoleId === 'moonchild') {
      ...
      this.getPendingMoonchildDeaths().push({
        ...
        targetId: target.playerId,
        targetWasGood: targetRegistration.alignment === 'good',
    ...
    for (const action of orderedNightActions) {
      ...
      try {
        ...
        const result = processNightAction(action, allPlayers, this.gameState.phase === GamePhase.FIRST_NIGHT, {
          ...
          editionId: this.gameConfig.edition
        });
        ...
        if (result.success) {
          ...
          if (abilityWorks || shouldTrackButlerMaster) {
            ...
            // Drunk/poisoned players still spend a once-per-game ability when
            // they validly attempt to use it. Do not apply the real effects,
            // but retain the actor's own usage marker so they cannot try again
            // on a later night. This intentionally ignores all target reminders
            // and every other effect from the suppressed ability.
            this.applySuppressedNightUsageEffects(result.effects || {}, action);
          }
  ...
  /**
   * A drunk/poisoned once-per-game ability has no game effect, but the attempt
   * still consumes that ability. Keep only the acting player's generic usage
   * reminder and discard every target/global effect generated by the skill.
   */
  private applySuppressedNightUsageEffects(effects: any, action: NightAction): void {
    if (!Array.isArray(effects?.reminders)) return;

    for (const reminder of effects.reminders) {
      const usageReminder = reminder?.reminder;
      if (
        reminder?.playerId !== action.playerId ||
        (usageReminder !== 'No ability' && usageReminder !== 'Philosopher used')
      ) {
        continue;
      }
      const actor = this.gamePlayers.get(action.playerId);
      if (actor) {
        this.addReminder(actor, usageReminder);
      }
      break;
    }
  }
