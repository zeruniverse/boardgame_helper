...
function translateRole(role) {
  return {
    ...
    team: role.team, // 保持原始字符串，稍后在生成时处理
...
// 将团队字符串转换为枚举
function teamToEnum(teamStr) {
  switch(teamStr) {
    case 'townsfolk': return 'Team.TOWNSFOLK';
    case 'outsider': return 'Team.OUTSIDER';
    case 'minion': return 'Team.MINION';
    case 'demon': return 'Team.DEMON';
    case 'traveler': return 'Team.TRAVELER';
    default: return teamStr;
  }
}

// 生成角色的 TypeScript 代码
function generateRoleCode(role) {
  const teamEnum = teamToEnum(role.team);
  return `  {
    id: "${role.id}",
    name: "${role.name}",
    edition: "${role.edition}",
    team: ${teamEnum},
    firstNight: ${role.firstNight},
    firstNightReminder: "${role.firstNightReminder.replace(/"/g, '\\"')}",
    otherNight: ${role.otherNight},
    otherNightReminder: "${role.otherNightReminder.replace(/"/g, '\\"')}",
    reminders: [${role.reminders.map(r => `"${r}"`).join(', ')}],
    ${role.remindersGlobal ? `remindersGlobal: [${role.remindersGlobal.map(r => `"${r}"`).join(', ')}],` : ''}
    setup: ${role.setup},
    ability: "${role.ability.replace(/"/g, '\\"')}",
    isCustom: false
  }`;
}
...
${rolesByEdition.traveler.map(role => generateRoleCode(role)).join(',\n')}
];

// 所有角色数据
export const ROLES: Role[] = [
  // Trouble Brewing
${rolesByEdition.tb.map(role => generateRoleCode(role)).join(',\n')},
  
  // Bad Moon Rising
${rolesByEdition.bmr.map(role => generateRoleCode(role)).join(',\n')},
  
  // Sects & Violets
${rolesByEdition.snv.map(role => generateRoleCode(role)).join(',\n')},
...
// 写入文件
fs.writeFileSync(
  path.join(__dirname, '../backend/src/utils/botcData.ts'),
  tsContent
);

console.log('修复的 BOTC 数据文件已生成！'); 